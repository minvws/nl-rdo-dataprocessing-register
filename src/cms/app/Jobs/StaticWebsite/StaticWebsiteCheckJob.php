...
class StaticWebsiteCheckJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(
        ...
        $logger->info('Static website publication checks starting');

        try {
            $staticWebsiteCheckService->check();
        } catch (StaticSitemapCheckJsonException $staticSitemapCheckJsonException) {
            $logger->notice('Static website publication checks failed', ['message' => $staticSitemapCheckJsonException->getMessage()]);
        } catch (StaticSitemapCheckRequestException $staticSitemapCheckRequestException) {
            $logger->info('Static website publication checks retry', ['message' => $staticSitemapCheckRequestException->getMessage()]);
            $this->release(60);
        }

        $logger->info('Static website publication checks done');
    }
}
