...
class PublicWebsiteCheckForcedJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(
        ...
        $logger->info('Public website publication forced checks starting');

        try {
            $publicWebsiteCheckService->checkForced();
        } catch (PublicSitemapCheckJsonException $publicSitemapCheckJsonException) {
            $logger->notice(
                'Public website publication forced checks failed',
                ['message' => $publicSitemapCheckJsonException->getMessage()],
            );
        } catch (PublicSitemapCheckRequestException $publicSitemapCheckRequestException) {
            $logger->info(
                'Public website publication forced checks retry',
                ['message' => $publicSitemapCheckRequestException->getMessage()],
            );
            $this->release(60);
        }

        $logger->info('Public website publication forced checks done');
    }
}
